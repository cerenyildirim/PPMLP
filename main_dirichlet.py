...
for rn_glb in range(5):
    ...
    print(
            bcolors.HEADER
            + f" >> Participant # = {PARTICIPANT_NUM}"
            + bcolors.ENDC
        )

    # Output the generated parameters
    print(  bcolors.OKCYAN
            + f"p = {cc.GetPlaintextModulus()}"
            + bcolors.ENDC)
    print(  bcolors.OKCYAN
            + f"n = {cc.GetCyclotomicOrder()/2}"
            + bcolors.ENDC)
    print(  bcolors.OKCYAN
            + f"lo2 q = {log2(cc.GetModulus())}"
            + bcolors.ENDC)
    print(  bcolors.OKCYAN
            + f"Batch size = {cc.GetRingDimension()/2}"
            + bcolors.ENDC)
    ...
    while min_size < 10:
        idx_batch = [[] for _ in range(PARTICIPANT_NUM)]
        for k in range(NUM_CLASSES):
            idx_k = np.where(y_train == k)[0] # holds the index of class k elements
            np.random.shuffle(idx_k)
            proportions = np.random.dirichlet(np.repeat(alpha, PARTICIPANT_NUM))
            ## Balance
            proportions = np.array([p*(len(idx_j)<N/PARTICIPANT_NUM) for p,idx_j in zip(proportions,idx_batch)])
            proportions = proportions/proportions.sum()
            proportions = (np.cumsum(proportions)*len(idx_k)).astype(int)[:-1]
            idx_batch = [idx_j + idx.tolist() for idx_j,idx in zip(idx_batch,np.split(idx_k,proportions))]
            min_size = min([len(idx_j) for idx_j in idx_batch])
        print([len(idx_j) for idx_j in idx_batch])
        total_size = sum([len(idx_j) for idx_j in idx_batch])
    ...
    for net_idx, indices in net_dataidx_map.items():
        # Collect the samples from X_train for the current subset
    ...
    for rn in range(ROUND_NUM):    # Warm-up
        if rn == 5:
            enc_time = 0
            server_eval_time = 0
            dec_time = 0
        ...
        for i in range(PARTICIPANT_NUM):
            ...
            for j in range(steps):
                weight_final.append(weights[j] * len(X_train_subsets[i]) / total_size)
            ...
            client_eval_time += (enc_end_time - client_eval_start_time)
        ...
        # server_eval_end_time = time.process_time()
    ...
    file_name = "ct_global_acc_values_dirich.txt"
    ...
    print(f" >> Encode/Encrypt: {enc_time/(ROUND_NUM*PARTICIPANT_NUM)}")
    print(f" >> (Per) Client Evalutation: {client_eval_time/(ROUND_NUM*PARTICIPANT_NUM)}")
    print(f" >> Server Evalutation: {server_eval_time/ROUND_NUM}")
    print(f" >> Decryption: {dec_time/(ROUND_NUM*PARTICIPANT_NUM) + (dec_fus_time/ROUND_NUM)}")
