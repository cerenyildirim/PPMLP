...
# # Count and print each class element number in each subset
# for i, y_subset in enumerate(y_train_subsets):
#     class_counts = np.sum(y_subset, axis=0)
#     print(f"Subset {i + 1} class counts: {class_counts}")
...
for rn in range(ROUND_NUM):    # Warm-up
    ...
    for i in range(PARTICIPANT_NUM):
        ...
        for j in range(steps):
            weight_final.append(weights[j] / PARTICIPANT_NUM)
...
file_name = "ct_global_acc_values_iid.txt"
