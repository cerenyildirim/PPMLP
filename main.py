...
if __name__ == "__main__":
    ...
    if partition == 0:
         # ! Split the classes among parties: IID
        ...
        # ! Split the classes among parties: Non-IID-Dirichlet
        min_size = 0
        total_size = 0
        alpha = 0.5
        N = y_train.shape[0]
    ...
    for rn in range(ROUND_NUM):
        ...
        for i in range(PARTICIPANT_NUM):
            ...
            for j in range(steps):
                weight_final.append(weights[j] * len(X_train_subsets[i]) / len(x_train))
